...
class TestCache:
    ...
    network required. Each test gets an isolated cache root.
    """
    ...
    def test_clear_cache_via_module_helper(
        ...
        """``_cache.clear_cache("calendar")`` wipes the entire calendar
        cache. No public method on AcademicCalendar needed."""
        ...
        removed = _cache.clear_cache("calendar")
        assert not (isolated_cache / "calendar").exists() \
    ...
    def test_clear_cache_when_no_cache_exists(self, isolated_cache):
        from sustech_survival import _cache
        assert _cache.clear_cache("calendar") == 0
        assert _cache.clear_cache("nonexistent_module") == 0
