...
class TestClearCache:
    def test_wipes_everything_for_module(self, tmp_path):
        # Populate two modules under tmp_path.
        _cache.save_json(_cache.cache_path("mod_a", "x.json", root=tmp_path), {"n": 1})
        _cache.save_json(_cache.cache_path("mod_a", "sub", "y.json", root=tmp_path),
                         {"n": 2})
        _cache.save_json(_cache.cache_path("mod_b", "z.json", root=tmp_path), {"n": 3})
        # Wipe only mod_a.
        removed = _cache.clear_cache("mod_a", root=tmp_path)
        assert removed == 2
        assert not _cache.cache_path("mod_a", root=tmp_path).exists()
        # mod_b untouched.
        assert _cache.cache_path("mod_b", "z.json", root=tmp_path).exists()

    def test_no_op_when_module_unknown(self, tmp_path):
        assert _cache.clear_cache("never_existed", root=tmp_path) == 0
...
class TestCacheRootKwarg:
    """Active cache root precedence (post-env-var cleanup):

    1. ``root=`` kwarg on ``tmp_root`` / ``cache_path`` / ``clear_cache``.
    2. ``config.json`` ``paths.cache`` (per-host user setting, no env var).
    3. Default: ``<home>/.sustech_survival/cache`` (where ``<home>`` is
       ``$SUSTECH_HOME`` or the user's actual home).
    """
    ...
    def test_config_root_default_is_home_dotdir(self, monkeypatch, tmp_path):
        """config_root() defaults to ~/.sustech_survival."""
        monkeypatch.delenv("SUSTECH_HOME", raising=False)
        monkeypatch.setattr(_cache, "user_home", lambda: tmp_path)
    ...
    def test_paths_cache_config_override_absolute(self, tmp_path, monkeypatch):
        """``config.json`` ``paths.cache`` (absolute) overrides the default
        cache root. ``SUSTECH_HOME`` is honoured for the config dir itself.
        """
        monkeypatch.delenv("SUSTECH_HOME", raising=False)
        monkeypatch.setattr(_cache, "user_home", lambda: tmp_path)
        monkeypatch.setattr(_cache, "config_file",
                            lambda root=None: tmp_path / ".sustech_survival" / "config.json")
        _cache.save_json(tmp_path / ".sustech_survival" / "config.json",
                         {"paths": {"cache": str(tmp_path / "abs_cache")}})
        assert _cache.tmp_root() == tmp_path / "abs_cache"
        assert _cache.cache_path("foo") == tmp_path / "abs_cache" / "foo"

    def test_paths_cache_config_override_relative_resolves_against_cwd(
        self, tmp_path, monkeypatch
    ):
        """``paths.cache`` is relative → resolve against cwd, not home."""
        monkeypatch.delenv("SUSTECH_HOME", raising=False)
        monkeypatch.setattr(_cache, "user_home", lambda: tmp_path / "home")
        monkeypatch.setattr(_cache, "config_file",
                            lambda root=None: tmp_path / ".sustech_survival" / "config.json")
        _cache.save_json(tmp_path / ".sustech_survival" / "config.json",
                         {"paths": {"cache": "rel_cache"}})
        monkeypatch.chdir(tmp_path)
        assert _cache.tmp_root() == tmp_path / "rel_cache"

    def test_explicit_root_kwarg_beats_config(self, tmp_path, monkeypatch):
        """``root=`` kwarg wins over config.json paths.cache — the escape
        hatch for tests and one-shot scripts."""
        monkeypatch.setattr(_cache, "user_home", lambda: tmp_path)
        monkeypatch.setattr(_cache, "config_file",
                            lambda root=None: tmp_path / ".sustech_survival" / "config.json")
        _cache.save_json(tmp_path / ".sustech_survival" / "config.json",
                         {"paths": {"cache": str(tmp_path / "config_dir")}})
    ...
    def test_clear_cache_honours_root_kwarg(self, tmp_path):
        """``clear_cache("sel", root=tmp_path)`` wipes exactly that root."""
        target = tmp_path / "sel" / "f.json"
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_text("{}", encoding="utf-8")
        n = _cache.clear_cache("sel", root=tmp_path)
        assert n == 1
