...
var ROW_HEIGHT = 38;
// The separated grid layout (tis.html: `table.grid{border-collapse:separate;
// border-spacing:1px}`) puts 1px of spacing between every cell box. Blocks
// that span periods bridge that gap per internal row so they render as one
// continuous band — keep this in sync with the CSS `border-spacing` value.
var GRID_GAP = 1;
      ...
      if (entries && entries.length) {
        ...
        if (hasStart) {
          ...
          entries.forEach(function(e) {
            ...
            var l = e.col * w;
            var z = 10 - e.col;
            var spanH = e.span * ROW_HEIGHT + (e.span - 1) * GRID_GAP;
          ...
          // Continuation row of a spanning block: drop this cell's top/bottom
          // borders (CSS .blk-mid) so the overflowing block reads as one
          // continuous band instead of being sliced at each row boundary.
          var c2 = 'cell blk-mid' + (isBlocked ? ' blocked' : '');
